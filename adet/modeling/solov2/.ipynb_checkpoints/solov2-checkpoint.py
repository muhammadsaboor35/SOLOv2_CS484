...
from .loss import dice_loss, sigmoid_focal_loss, FocalLoss
...
class SOLOv2(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.class_balance_method = 'ins'
        self.ens_beta = 0.9999

        # image transform
        pixel_mean = torch.Tensor(cfg.MODEL.PIXEL_MEAN).to(self.device).view(3, 1, 1)
        pixel_std = torch.Tensor(cfg.MODEL.PIXEL_STD).to(self.device).view(3, 1, 1)
        self.normalizer = lambda x: (x - pixel_mean) / pixel_std
        self.adetlogger = logging.getLogger("adet.trainer")
        self.to(self.device)
    ...
    def get_ground_truth(self, gt_instances, mask_feat_size=None):
        ...
        for img_idx in range(len(gt_instances)):
            if gt_instances[img_idx].gt_classes.shape[0] == 0:
               continue
    ...
    def loss(self, cate_preds, kernel_preds, ins_pred, targets):
        #self.adetlogger.info("Starting training from iteration {}".format(start_iter))
        pass
        ...
        frequency = np.array([2633, 713, 1575, 530, 508, 598, 521, 352, 153, 302, 268, 727, 131, 846, 125, 1091, 2203, 728, 320, 228, 210, 112])
        balanced_weights = self.get_balanced_weights(self.num_classes,frequency,flatten_cate_labels_oh,self.class_balance_method)

        loss_cate = self.focal_loss_weight * sigmoid_focal_loss(flatten_cate_preds, flatten_cate_labels_oh,
                                    balanced_weights,
    ...
    def get_ens_weights(self,N,frequency):
        den = 1.0 - np.power(self.ens_beta,frequency)
        num = 1.0 - self.ens_beta
        E_n = num/den
        return (E_n*N)/np.sum(E_n)
    
    def get_ins_weights(self,N,frequency,power=1):
        I_n = 1.0/(np.power(frequency,power))
        return (I_n*N)/np.sum(I_n)

    def get_balanced_weights(self,N,frequency,gts,method):
        gts = gts.to('cpu').numpy()

        if method == 'ens':
            weights = self.get_ens_weights(N,frequency)
        elif method == 'ins':
            weights = self.get_ins_weights(N,frequency)
        elif method == 'isns':
            weights = self.get_ins_weights(N,frequency,0.5)

        weights = torch.tensor(weights).float()
        weights = weights.unsqueeze(0)
        weights = torch.tensor(weights.repeat(gts.shape[0],1)*gts)
        weights = weights.sum(1).unsqueeze(1).repeat(1,N)
